...
export class ByteArray {
  ...
  protected $endian: EndianConst = EndianConst.BIG_ENDIAN
  ...
  constructor(buffer?: ArrayBuffer | Uint8Array, bufferExtSize = 0) {
    ...
    if (buffer) { // 有数据，则可写字节数从字节尾开始
      ...
      if (bufferExtSize === 0) {
        bytes = new Uint8Array(pos)
      }
      else {
        const multi = (pos / bufferExtSize | 0) + 1
   ...
   */
  public _writeUint8Array(bytes: Uint8Array | ArrayLike<number>, validateBuffer = true): void {
    const pos = this._position
    ...
    while (data.length > pos) {
      ...
      if (_byte === this.EOF_byte) {
        ...
        if (utf8_bytes_needed === 0) {
          ...
          utf8_bytes_seen += 1
          utf8_code_point = utf8_code_point + (_byte - 0x80) * 64 ** (utf8_bytes_needed - utf8_bytes_seen)
